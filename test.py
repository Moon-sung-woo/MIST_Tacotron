...
def get_split_text(file_path):
    f = open(file_path, 'r', encoding='utf-8')
    lines = f.readlines()
    f.close()

    lines_len = len(lines)
    line1 = lines[:lines_len//3]
    line2 = lines[lines_len//3 : (lines_len*2)//3]
    line3 = lines[(lines_len*2)//3:]
...
# 멀티 프로세싱 하기위해 나눠주는 부분
file_path = 'filelists/train_mel_file_list_1.txt'
line1, line2, line3 = get_split_text(file_path)
line_list = [line1, line2, line3]

# make_style_png((line1))

# 멀티 프로세싱 하는 부분
pool = multiprocessing.Pool(processes=3) # 현재 시스템에서 사용 할 프로세스 개수
pool.map(make_style_png, line_list) #함수, 함수에서 쓰이는 값들
pool.close()
pool.join()
